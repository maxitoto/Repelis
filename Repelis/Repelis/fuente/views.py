...
from django.db.models import Q #esto da alguna funcionalidades lógicas para hacer consultas,


class ViewBusqueda(ListView):
    template_name = "mostrarBusqueda.html"
    ...
    def get_queryset(self):
        ...
        if search_type == "Película":
            return Pelicula.objects.filter(nombre__icontains=query)
        elif search_type == "Actor":
            return Artista.objects.filter(tipo_de_Artista='ACTOR').filter(Q(nombre__icontains=query) | Q(apellido__icontains=query))
        elif search_type == "Director":
            return Artista.objects.filter(tipo_de_Artista='DIRECTOR').filter(Q(nombre__icontains=query) | Q(apellido__icontains=query))
        elif search_type == "Buscar":
            resultados_peliculas = Pelicula.objects.filter(nombre__icontains=query)
            resultados_artistas = Artista.objects.filter(Q(nombre__icontains=query) | Q(apellido__icontains=query)).distinct()
        
            return list(chain(resultados_peliculas, resultados_artistas))
